...
// Bookings This Week (Monday to Sunday)
$stmt_this_week = $pdo->prepare("SELECT COUNT(*) FROM appointments WHERE org_id = ? AND YEARWEEK(start_time, 1) = YEARWEEK(CURDATE(), 1) AND status = 'Scheduled'");
$stmt_this_week->execute([$current_org_id]);
$bookings_this_week = $stmt_this_week->fetchColumn();

// Bookings Last Week
$stmt_last_week = $pdo->prepare("SELECT COUNT(*) FROM appointments WHERE org_id = ? AND YEARWEEK(start_time, 1) = YEARWEEK(CURDATE() - INTERVAL 7 DAY, 1) AND status = 'Scheduled'");
$stmt_last_week->execute([$current_org_id]);
$bookings_last_week = $stmt_last_week->fetchColumn();
...
<div class="metrics-grid">
    ...
    <div class="metric-card blue">
        ...
        <div class="metric-value"><?php echo $bookings_this_week; ?></div>
        <span class="metric-action" style="color: #3B82F6;">Current Week</span>
    </div>

    <div class="metric-card" style="border-left: 4px solid #64748b;">
        <div class="metric-label">📊 Bookings Last Week</div>
        <div class="metric-value"><?php echo $bookings_last_week; ?></div>
        <span class="metric-action" style="color: #64748b;">Previous Week</span>
    </div>
</div>
