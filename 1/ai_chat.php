...
function callAIAPI($api_url, $api_key, $message) {
    ...
    if ($http_code === 200 && $response) {
        // Prefer parsing SSE stream: return the final "done.response"
        $lines = preg_split("/\r\n|\n|\r/", $response);
        foreach ($lines as $line) {
            ...
            if (is_array($parsed) && ($parsed['type'] ?? '') === 'done') {
                $done = $parsed['response'] ?? '';
                if (is_string($done) && trim($done) !== '') {
                    return trim($done);
                }
                $blocks = $parsed['content_blocks'] ?? null;
                if (is_array($blocks)) {
                    foreach ($blocks as $b) {
                        if (($b['type'] ?? '') === 'text' && isset($b['content'])) {
                            return trim((string)$b['content']);
                        }
                    }
                }
            }
        ...
        // Fallback: if the server didn't stream, try to parse as JSON
        $maybe = json_decode($response, true);
        if (is_array($maybe) && isset($maybe['response'])) {
            return trim((string)$maybe['response']);
        }

        // Last fallback: strip SSE prefixes and return plain text
        $clean = [];
        foreach ($lines as $line) {
            $line = trim($line);
            if ($line === '') continue;
